...
export default function NetworkIntelligence() {
  ...
  return (
    // Full viewport — no padding from parent, use negative margin to break out
    <div className="fixed inset-0 z-0" style={{ background: '#020617' }}>
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <span className="flex items-center gap-2 rounded-full border border-rose-500/30 bg-rose-500/10 px-4 py-2 text-sm font-semibold text-rose-400">
      ...
      <motion.div
        ...
        className="absolute bottom-32 left-8 z-10 flex flex-col gap-3 sm:flex-row"
      >
        {[
          ...
          <div key={s.label}
            className="rounded-xl border border-white/5 px-5 py-3"
      ...
      >
        ...
        >
          ...
          <div>
            <div className="text-sm font-semibold">Attack Origins</div>
            <div className="text-[11px] text-slate-500">Country breakdown</div>
          </div>
          <ChevronRight className={`ml-2 h-4 w-4 transition-transform ${panel === 'origins' ? 'rotate-90' : ''}`} />
        </button>
        ...
        >
          ...
          <div>
            <div className="text-sm font-semibold">Fraud Ring Graph</div>
            <div className="text-[11px] text-slate-500">Mule network map</div>
          </div>
          <ChevronRight className={`ml-2 h-4 w-4 transition-transform ${panel === 'fraudring' ? 'rotate-90' : ''}`} />
        </button>

        <button
          onClick={() => setPanel(null)}
          className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-5 py-4 text-left text-slate-400 transition-all hover:border-white/20 hover:bg-white/10"
          style={{ backdropFilter: 'blur(12px)' }}
        >
          <Activity className="h-5 w-5 shrink-0" />
          <div>
            <div className="text-sm font-semibold">Clear Panels</div>
            <div className="text-[11px] text-slate-500">Globe only view</div>
          </div>
        </button>
      </motion.div>
